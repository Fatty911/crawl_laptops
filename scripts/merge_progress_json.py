...
def merge_progress(current: dict[str, Any], ours: dict[str, Any], theirs: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("crawled_phones", "processed_phones", "crawled_pages", "processed_ids", "crawled_items"):
    ...
    if "total_items" in merged:
        merged["total_items"] = max_number(
            current.get("total_items"),
            ours.get("total_items"),
            theirs.get("total_items"),
            default=len(merged.get("processed_ids", [])),
        )
