...
def is_semantically_valid_record(payload, expected_id):
    ...
    raw_url = str(payload.get("url") or payload.get("产品链接") or "").strip()
    release_text = " ".join(
...
def _read_raw_files(archive_bytes):
    ...
    if not raw_files:
        raise InvalidCacheArtifact("artifact contains no raw laptop cache")
    return raw_files
...
def _github_request(url, token, *, accept="application/vnd.github+json"):
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "crawl-laptops-pconline-cache-restore",
        },
    )
